...
<dsp:page>
	...
	<body>
	<html>
	<p><span class=storebig><b>Order Information</span><br>
	<p><dsp:form action="" method="POST">
		<dsp:droplet name="/atg/dynamo/droplet/Switch">
			<dsp:param bean="ShoppingCartModifier.formError" name="value" />
			<dsp:oparam name="true">
				<span class=registrationerror> <span class=help>checkout
				Error !:</span>
				<p>
				<UL>
...
--%>
    ...
    %>
				...
				<dsp:input
					...
					maxlength="10" size="10" type="text" required="<%=true%>" /> <br>
			...
			<tr valign=top>
				<td width=200><span class=help><b>Decide if you wish
				to ship all items to your billing address or a different address.</span></td>
				<td></td>
				<td>
				<table width=100% cellpadding=0 cellspacing=0 border=0>
					<tr>
						<td class=box-top-store><b>My shipping address</td>
					</tr>
				</table>
     ...
     edited) by the user. --> <dsp:input
					bean="ShoppingCartModifier.shippingGroup.shippingAddress.ownerId"
					beanvalue="Profile.id" type="hidden" /> <dsp:input
...
--%>
    ...
    %>
				...
				<b>Shipping Method :</b> <dsp:valueof
					bean="ShoppingCartModifier.shippingGroup.shippingMethod"></dsp:valueof><br>
				<p>&nbsp;<br>
				</td>
			</tr>

			<tr valign=top>
				<td width=200><span class=help><b>We accept credit
				card payments in the form of master card, visa, american express,
				and discover. </span></td>
				<td></td>
				<td>
				<table width=100% cellpadding=0 cellspacing=0 border=0>
					<tr>
						<td class=box-top-store><b>Payment method</td>
					</tr>
				</table>
				<p>I will pay with this credit card
				<blockquote>
				<p>New card type <dsp:select
	...
	<dsp:option value="Discover" />Discover
				...
				<dsp:input bean="ShoppingCartModifier.paymentGroup.creditCardNumber"
					maxlength="16" size="16" type="text" required="<%=true%>" /><br>

				<!-- Set the month that the card will expire on --> Month: <dsp:select
	...
	<dsp:option value="12" />December
	</dsp:select> <!-- Set the year that the card will expire on --> Year: <dsp:select
				...
				<dsp:input bean="ShoppingCartModifier.moveToConfirmationSuccessURL"
					type="hidden" value="../checkout/order_confirm.jsp" /> <!-- Set the Location to go to if there are error on this form.  We stay on this page and then -->
				<!-- just display the error messages.                                                          -->
				<dsp:input bean="ShoppingCartModifier.moveToConfirmationErrorURL"
					type="hidden" value="order_info.jsp" /> <!-- Go to this URL if user's session expired while he pondered this page -->
				<dsp:input bean="ShoppingCartModifier.sessionExpirationURL"
					type="hidden" value="../index.jsp" /> <dsp:input
