...
<tr valign=top>
	<td><dsp:include page="YourItems.jsp"></dsp:include></td>
	<td></td>
	<td>

	<table width=100% cellpadding=0 cellspacing=0 border=0>
		<tr>
			<td class=box-top-store>Ship your items</td>
		</tr>
	</table>
	<p><%-- If user has a shipping address, that's where it's going to go by default.
  If there is no shipping address yet, then we have the form to enter one. --%>

	<% Boolean noShippingAddr = Boolean.FALSE; %> <dsp:getvalueof
		id="shipAddress" bean="Profile.shippingAddress">
		<%
   if (null == shipAddress) {
 ...
 %>
	</dsp:getvalueof> <core:switch value="<%=noShippingAddr%>">
		...
		<core:case value="<%= Boolean.FALSE %>">
			...
			<dsp:input
				...
				beanvalue="Profile.shippingAddress.ownerId" size="20" type="hidden" />
		
		Your items will be shipped to your default shipping address, unless you
		choose otherwise.
		<p><dsp:include page="../../user/DisplayAddress.jsp">
				<dsp:param name="address" bean="Profile.shippingAddress" />
			</dsp:include>
			<p><input type=submit value="Ship somewhere else">
		</core:case>
...
<tr>
	...
	<%/* Get a listing of the shipping methods that a user can select */%> <dsp:droplet
		...
		<dsp:oparam name="output">
			<dsp:select bean="ShoppingCartModifier.shippingGroup.shippingMethod">
				<core:forEach id="methods"
					values='<%= dynrequest.getObjectParameter("availableShippingMethods") %>'
					castClass="String" elementId="method">
					<dsp:option value="<%=method%>" />
					<%=method%>
				</core:forEach>
			</dsp:select>
		</dsp:oparam>
